...
│  Frontend only                        sujal/MediBot      │
│  /api/proxy/* → MediBot              Full app (FE + BE)     │
└──────────┬──────────────────────────────┬───────────────────┘
           │                              │
           ▼                              ▼
┌─────────────────────────────────────────────────────────────┐
│  MediBot Space (sujal/MediBot)                           │
│  Next.js 14 + SQLite                                        │
│                                                             │
...
# 3. Verify
curl https://sujal-medibot.hf.space/api/health
# → {"status":"ok"}
```
...
- Force-pushes to `sujal/MediBot` HF Space
...
| `NEARBY_URL` | `https://sujal-metaengine-nearby.hf.space` | Default works |
| `SCANNER_URL` | `https://sujal-medicine-scanner.hf.space` | Default works |
...
# Verify
curl https://sujal-medicine-scanner.hf.space/api/health
...
**Test the scanner:**
```bash
curl -X POST https://sujal-medicine-scanner.hf.space/api/scan \
...
# Verify
curl https://sujal-metaengine-nearby.hf.space/api/health
...
**Test the finder:**
```bash
curl -X POST https://sujal-metaengine-nearby.hf.space/api/search \
...
| Variable | Value |
|----------|-------|
| `NEXT_PUBLIC_BACKEND_URL` | `https://sujal-medibot.hf.space` |
...
| Application URL | Public URL (used in emails) | `https://sujal-medibot.hf.space` |
...
| `APP_URL` | No | `https://sujal-medibot.hf.space` | Public URL |
| `ALLOWED_ORIGINS` | No | `*` | CORS allowed origins |
| `NEARBY_URL` | No | `https://sujal-metaengine-nearby.hf.space` | Nearby finder URL |
| `SCANNER_URL` | No | `https://sujal-medicine-scanner.hf.space` | Scanner URL |
...
| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `NEXT_PUBLIC_BACKEND_URL` | Yes | `https://sujal-medibot.hf.space` | MediBot Space URL |
...
1. Check the Scanner Space is running: visit [huggingface.co/spaces/sujal/Medicine-Scanner](https://huggingface.co/spaces/sujal/Medicine-Scanner)
...
1. Check MetaEngine Space is running: visit [huggingface.co/spaces/sujal/MetaEngine-Nearby](https://huggingface.co/spaces/sujal/MetaEngine-Nearby)
...
2. If still timing out, wake MediBot first: visit [huggingface.co/spaces/sujal/MediBot](https://huggingface.co/spaces/sujal/MediBot) and wait ~30s
...
1. Check Space logs at `huggingface.co/spaces/sujal/SpaceName` → Logs tab
2. Common issues:
...
| `HF_USERNAME` | `sujal` (your HF username) |
